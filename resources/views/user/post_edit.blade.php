...
@section('title', "User's post edit page")
...
<div class=' d-flex justify-content-center border border-2 rounded border-warning p-2'>
    ...
    <form method="POST" action="{{ route('user_post_edit',['user_id'=>$post_detail->user_id,'post_id'=>$post_detail['id']]) }}" enctype="multipart/form-data" class='border border-1 p-5 rounded mx-2'>
        ...
        <div class="row">
            ...
            <x-input id="title" class="block mt-1 col-8" type="text" name="title" value="{{$post_detail['title']}}" required autofocus />
        </div>
        <div>
            <x-label for="description" :value="__('Description')" />

            <!-- <x-input id="description" class="block mt-1 w-full" type="text" name="description" :value="$post_detail['description']"  /> -->
            <textarea class="form-control col-8" id="description" name="description">{{$post_detail['description']}}</textarea>
        </div>
        ...
        <div class="row mt-2">
            ...
            <x-input id="price" class="block mt-1 w-full" type="text" name="price" :value="$post_detail['price']" required />
        </div>
        <div class="row mt-2">
            ...
            <select id="category_id" name="category_id">
                ...
                <option value="{{$cat['id']}}" @if($cat['id']==$post_detail['category_id']) selected @endif>{{$cat['name']}}</option>
                @endforeach
