    ...
    <div>
        <div class=" d-flex justify-content-center my-2">
            ...
            <ul class="list-inline">
                ...
                <li class="btn btn-primary list-inline-item" @click="getinbox">Inbox({{num_inbox}}) </li>
                <li class="btn btn-primary  list-inline-item" @click="getoutbox">Outbox ({{num_outbox}})</li>
                <li class="btn btn-primary  list-inline-item" @click="sent_message">New Message</li>
            </ul>
        </div>
        ...
        <div v-if="display_messages==1" class="row col-12 mx-auto border rounded ">
                            <div class="col-4 border border-primary overflow-auto text-break">
                                ...
                                <p v-if="ck_inbox==1"><u>({{unread}}) Unread</u></p>
                                 <p v-if="ck_outbox==1"><u>Messages sented:</u></p>

                                
                                    <div v-for='(item,index) in list_messages' :key="index" v-bind:class="{'font-weight-bold': (!item.read_status&&ck_inbox==1)}"  @click="message_detail(item.id,$event)"> <p> {{item.title}} </p> <hr/>
                    ...
                    <div class="col-8 border border-primary ">
                        <div v-if="display_detail==1">
                            ...
                            <input class="btn btn-warning" value="Delete" @click="un_display(message_id_detail)"  />
                         </div>
                    </div>
        ...
        <div v-if="display_form==1">
             <form class="m-2 p-2 border border-warning rounded">
                    ...
                    <option v-for="(item,index) in list_users" :key="index" :value=item.id>{{item.username}}</option>
  ...
  components: { Input },
    ...
    methods:{
        ...
        get_unread(){
            ...
            axios.get('/user/messages/unread')
            .then(response => {
                // console.log("unread");
                //  console.log(response.data);
         ...
         getoutbox(){
            ...
            this.ck_outbox=1;
            this.ck_inbox=0;
            axios.get('/user/messages/outbox')
            .then(response => {
                // console.log("outbox");
                //  console.log(response.data);
                 this.list_messages=response.data;
        ...
        message_detail(message_id){
            this.display_detail=1;
             ...
             axios.get('/user/messages/'+message_id+'/detail')
            .then(response => {
                // console.log("outbox");
                //  console.log(response.data);
        ...
        save_message(){
            ...
            this.result_message='';
            if(this.receiver=='' || this.message_body=='' ||this.message_title==''){this.result_message='Please input info for all fields above'}
            else{

                // alert('OK');

           axios.post('/user/messages/save',{message: this.message_body,receiver_id:this.receiver, title:this.message_title})
            .then(response => {
        ...
        un_display(message_id){
            console.log("undisplay"+message_id);
             axios.get('/user/messages/'+message_id+'/un_display')
            .then(response => {
